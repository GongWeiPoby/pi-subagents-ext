...
import { truncateChars } from './core/common';
import {
  DELEGATE_DEFAULT_MAX_TOOL_CALLS,
  DELEGATE_DEFAULT_MAX_TURNS,
  DELEGATE_DEFAULT_TIMEOUT_SECONDS,
  DELEGATE_INLINE_ANSWER_BYTES,
} from './core/delegate/facade-contract';
import type { DelegateHookContractEvidence } from './core/delegate/hook-contract';
import {
...
export interface DelegateExtensionRuntime {
  ...
  resolveDelegateRoute: typeof import('./core/delegate/launch').resolveDelegateRoute;
  prepareDelegateLaunch: (
    input: Parameters<typeof import('./core/delegate/runner').prepareDelegateLaunch>[0],
  ) => Promise<DelegatePreparedLaunch>;
}

interface DelegateResultRuntime {
  decideDelegateDelivery: typeof import('./core/delegate/runner').decideDelegateDelivery;
  evaluateDelegateTerminal: typeof import('./core/delegate/runner').evaluateDelegateTerminal;
  inlineTooLarge: typeof import('./core/delegate/runner').inlineTooLarge;
}

interface FusionResultRuntime {
  readFusionCommittedResult: typeof import('./core/fusion/result-package').readFusionCommittedResult;
  readFusionFailureResult: typeof import('./core/fusion/result-package').readFusionFailureResult;
}

async function importDelegateExtensionRuntime(): Promise<DelegateExtensionRuntime> {
  const [launch, runner] = await Promise.all([
    import('./core/delegate/launch'),
    import('./core/delegate/runner'),
  ]);
  return {
...
async function importDelegateResultRuntime(): Promise<DelegateResultRuntime> {
  const runner = await import('./core/delegate/runner');
  return {
...
async function importFusionResultRuntime(): Promise<FusionResultRuntime> {
  const resultPackage = await import('./core/fusion/result-package');
  return {
