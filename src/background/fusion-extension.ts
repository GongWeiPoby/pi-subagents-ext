...
import type { BuiltFusionCleanTaskCanonicalInput } from './core/fusion/clean-context';
import type {
  BuildFusionCanonicalInputOptions,
  BuiltFusionCanonicalInput,
} from './core/fusion/context';
import { CURRENT_MODEL_SELECTION } from './core/fusion/facade-contract';
import type { FusionOrchestrator } from './core/fusion/orchestrator';
import { canonicalizeFusionPublicUrl } from './core/fusion/source-policy';
import {
  cloneFusionUsage,
...
} from './core/fusion/workflows';
import { LazyModule, SynchronousActivationCloseFence } from './core/lazy-module';
import type { FusionModelChoice, FusionModelSelectorResult } from './ui/fusion-model-selector';
...
export interface FusionExecutionRuntime {
  buildFusionCanonicalInput: typeof import('./core/fusion/context').buildFusionCanonicalInput;
  buildCleanFusionCanonicalInput: typeof import('./core/fusion/clean-context').buildCleanFusionCanonicalInput;
  loadFusionModelConfig: typeof import('./core/fusion/config').loadFusionModelConfig;
  resolveFusionModels: typeof import('./core/fusion/config').resolveFusionModels;
  orchestrator: Pick<FusionOrchestrator, 'run'>;
}

export interface FusionModelSelectorRuntime {
  fusionModelConfigPath: typeof import('./core/fusion/config').fusionModelConfigPath;
  loadFusionModelConfig: typeof import('./core/fusion/config').loadFusionModelConfig;
  saveFusionModelConfig: typeof import('./core/fusion/config').saveFusionModelConfig;
  createSelector: (
    options: ConstructorParameters<
      typeof import('./ui/fusion-model-selector').FusionModelSelector
    >[0],
  ) => InstanceType<typeof import('./ui/fusion-model-selector').FusionModelSelector>;
}
...
async function importFusionExecutionRuntime(): Promise<FusionExecutionRuntime> {
  const [context, cleanContext, config, orchestratorModule] = await Promise.all([
    import('./core/fusion/context'),
    import('./core/fusion/clean-context'),
    import('./core/fusion/config'),
    import('./core/fusion/orchestrator'),
  ]);
  return {
...
async function importFusionModelSelectorRuntime(): Promise<FusionModelSelectorRuntime> {
  const [config, selector] = await Promise.all([
    import('./core/fusion/config'),
    import('./ui/fusion-model-selector'),
  ]);
  return {
...
function normalizeVerification(value: unknown): FusionVerificationRequest {
  ...
  const evidence = Object.hasOwn(value, 'evidence')
    ...
    : [];
  const hasReason =
    Object.hasOwn(value, 'reason') && value['reason'] !== undefined;
...
export function prepareFusionValidateArguments(args: unknown): FusionValidateRequest {
  ...
  if (Object.hasOwn(record, 'prompt')) {
    throw new Error(
