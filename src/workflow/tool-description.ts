...
There are no built-in agents. Every agent() call must select a user-defined opts.agentType unless the user explicitly configured defaultAgent in subagents.json. Examples below that omit agentType require that setting. Unknown types never fall back. Resume retains the original child's type. Do not invent or configure a default on the user's behalf.
...
Direct \`script\`, \`scriptPath\`, and \`name\` are mutually exclusive. Optional \`task_id\` binds the workflow controller to one pending structured Todo: the Todo stays in progress while the workflow runs, completes only when that workflow completes, and returns to pending with the workflow error when it fails or is killed. A Todo already claimed by TaskExecute or another workflow is refused; workflow children never inherit the binding. In an interactive UI, every direct invocation is previewed for confirmation, including an exact \`resumeFromRunId\`; authorization is never inferred from user prose, workflow names, or risk keywords. Interactive workflows containing nested \`workflow()\` behavior are rejected because a top-level preview cannot disclose the child. Trusted headless automation may compose one level through saved named workflows.
