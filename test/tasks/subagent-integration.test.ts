...
describe("TaskExecute", () => {
  ...
  it("reconciles an agent that completes before the spawn reply is handled", async () => {
    rpc.unsub();
    mock = mockPi();
    rpc = installSubagentsMock(mock.pi, { completeBeforeSpawnReply: "fast result" });
    ...
    globals[managerKey] = {
      getRecord: (id: string) => ({
        ...
        status: "completed",
        result: "fast result",
        taskExecutionRef: rpc.taskExecutionRef(id),
      }),
    };
    ...
    try {
      ...
      expect(task.content[0].text).toContain("fast result");
    } finally {
      delete globals[managerKey];
    }
  });
  ...
  it("atomically claims a shared task across extension instances", async () => {
    const directory = mkdtempSync(join(tmpdir(), "pi-tasks-claim-"));
    process.env.PI_TASKS = join(directory, "tasks.json");
    const first = mockPi();
    const second = mockPi();
    const firstRpc = installSubagentsMock(first.pi);
    const secondRpc = installSubagentsMock(second.pi);
    initExtension(first.pi as never);
    initExtension(second.pi as never);
    try {
      ...
      const [firstResult, secondResult] = await Promise.all([
        first.executeTool("TaskExecute", { task_ids: ["1"] }),
        second.executeTool("TaskExecute", { task_ids: ["1"] }),
      ]);

      expect(firstRpc.spawned.length + secondRpc.spawned.length).toBe(1);
      expect(`${firstResult.content[0].text}\n${secondResult.content[0].text}`)
        .toContain("not pending (status: in_progress)");
    } finally {
      firstRpc.unsub();
      secondRpc.unsub();
      rmSync(directory, { recursive: true, force: true });
...
describe("Completion listener", () => {
  ...
  it("marks task completed on subagents:completed event", async () => {
    ...
    // Simulate agent completion with the runtime's canonical execution ref.
    rpc.complete("agent-1");
  ...
  it("reverts task to pending on subagents:failed event", async () => {
    ...
    // Simulate agent failure with the runtime's canonical execution ref.
    rpc.fail("agent-1", "Out of turns");
  ...
  it("settles a truly metadata-only legacy agent task without a ref", async () => {
    await mock.executeTool("TaskCreate", {
      subject: "Legacy running task",
      description: "Desc",
    });
    // No TaskExecute call: this in-memory compatibility fixture has metadata
    // identifying an agent but no canonical execution binding.
  ...
  it("rolls back execution before unsubscribing on session shutdown", async () => {
    await mock.executeTool("TaskCreate", {
    ...
    const result = await mock.executeTool("TaskGet", { taskId: "1" });
    expect(result.content[0].text).toContain("Status: pending");
    expect(result.content[0].text).toContain("session shut down before execution settled");
...
describe("Standalone operation (no subagents extension)", () => {
  ...
  it("TaskExecute gracefully refuses without subagents", async () => {
    ...
    expect(text).toContain("won't follow them");
  });
...
describe("RPC protocol correctness", () => {
  ...
  it("surfaces a stop RPC error and keeps the task in progress", async () => {
    const mock = mockPi();
    const rpc = installSubagentsMock(mock.pi);
    initExtension(mock.pi as any);
    ...
    await expect(mock.executeTool("TaskStop", { task_id: "1" })).rejects.toThrow("Agent not found");
    const task = await mock.executeTool("TaskGet", { taskId: "1" });
  ...
  it("surfaces a stop RPC timeout and keeps the task in progress", async () => {
    const mock = mockPi();
    const rpc = installSubagentsMock(mock.pi);
    initExtension(mock.pi as any);
    ...
    // runtime handlers before stop so only the RPC timeout path remains.
    rpc.unsub();

    vi.useFakeTimers();
    const stopPromise = mock.executeTool("TaskStop", { task_id: "1" });
    const rejection = expect(stopPromise).rejects.toThrow("subagents:rpc:stop timeout");
...
describe("Protocol version mismatch", () => {
  ...
  it("old handler without a version warns about the bundled protocol", async () => {
    const mock = mockPi();
    ...
    expect(ctx.ui.notify).toHaveBeenCalledWith(
      expect.stringContaining("does not expose a compatible task-execution protocol"),
      "warning",
    );
  });
...
describe("Cascade data injection (buildTaskPrompt)", () => {
  ...
  it("injects prerequisite result into cascaded agent prompt", async () => {
    ...
    rpc.complete("agent-1", "The answer is 42");
  ...
  it("truncates long prerequisite results at 4KB", async () => {
    ...
    rpc.complete("agent-1", longResult);
