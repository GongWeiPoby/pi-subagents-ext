...
describe("Auto-cascade (enabled)", () => {
  ...
  it("keeps cascade options with the originating execution", async () => {
    await createAgentTask("Task A");
    await createAgentTask("Task B");
    await createAgentTask("Task C");
    await createAgentTask("Task D");
    await mock.executeTool("TaskUpdate", { taskId: "2", addBlockedBy: ["1"] });
    await mock.executeTool("TaskUpdate", { taskId: "4", addBlockedBy: ["3"] });
    ...
    expect(rpc.spawned[2].options.maxTurns).toBe(3);
  });
