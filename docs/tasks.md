...
This protected execution state is separate from `metadata`. `TaskCreate.metadata` and `TaskUpdate.metadata` remain user-extensible, except that the runtime-owned `workflowAggregate` key is stripped at both tool boundaries: users cannot create, replace, or null-delete it. Internal execution-coordinator updates can still persist that aggregate while holding the matching binding or stop reservation. `TaskOutput` reads the canonical kind and executor first, so a Todo ID follows its workflow controller rather than a stale agent ID; settled workflow and legacy metadata-only Agent records remain queryable through protected display metadata. TaskStore read APIs return deep clones, so mutating a `get`, `list`, `create`, `update`, or claim result cannot alter in-memory execution authority outside CAS. Existing persisted agent-backed tasks are migrated once using their stored attempt and agent IDs; the task file receives a stable store identity during that migration.

## TaskOutput views

`TaskOutput` keeps its existing behavior when `view` is omitted. `view: "summary"` is a metadata-only inspection: it reports task and executor identity, status, attempt/usage counters, workflow coverage, aggregate artifact status, and whether result-body persistence is enabled. It does not read or return a result body. `view: "children"` applies to workflow controllers and lists at most 100 bounded child-attempt records, including status, invocation, safe references, usage, and sanitized error markers; on an Agent or manual task it says that children are not applicable. `view: "result"` is the explicit opt-in body read. It returns persisted Markdown with ordinary newlines intact and only terminal/C0/C1/bidi controls removed, plus `offset`, `limit`, total length, and `hasMore`. Offsets and limits are non-negative integers, the default limit is 20,000 characters, and the hard limit is 8 MiB; an offset past the end returns an empty slice rather than a path error.

The result reader never accepts an artifact path. It resolves the fixed current-parent-session location inside a project namespace made from a bounded readable suffix of `resolve(cwd)` plus its full SHA-256 hash. That keeps formerly colliding paths such as `/tmp/a-b/c` and `/tmp/a/b-c` separate even with the same session ID, and the reader never falls back to the old unhashed directory form. It requires regular bounded manifest/body files, checks manifest artifact and Agent/workflow identity, validates a workflow Todo binding when one exists, and verifies the body SHA-256 digest. Missing, oversize, malformed, identity-mismatched, binding-mismatched, or digest-mismatched artifacts are rejected. With body persistence disabled, `view: "result"` returns the fixed `Output persistence disabled.` marker and does not expose the in-memory result.

After a reload, a settled workflow can be resolved from its persisted aggregate metadata and internal manifest. For `wf_*` lookups, a live canonical workflow binding wins, followed by a validated persisted `workflowAggregate.taskBinding`, then an unbound aggregate manifest; `metadata.workflowId` is consulted only for old tasks with no aggregate. Bound and unbound workflow result slices therefore remain available after reload within the same hashed project and parent-session locator. A live running/paused workflow, or any privacy-on run whose aggregate artifact is not `complete`, returns an explicit bounded unavailable state before the body reader runs and never falls back to the workflow's in-memory result. Agent reload is deliberately narrower: `view: "result"` needs the current in-memory Agent record's artifact locator. Running/queued Agents and `pending` Agent artifacts also return unavailable before a body read. If the record is gone after reload, the view reports unavailable rather than fabricating locator support from legacy task metadata.

This is not a general artifact API. There is no standalone `ArtifactRead` tool, callers cannot supply filesystem paths, and omitted/default or summary views never read a body. A successful explicit settled Agent/workflow body read consumes the existing completion notification; summary/children views, privacy markers, unavailable results, timeouts, and aborts do not. Workflow children remain bounded attempt records, not verification evidence or proof of task completion.
