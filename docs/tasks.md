...
`TaskOutput` keeps its existing behavior when `view` is omitted. `view: "summary"` is a metadata-only inspection: it reports task and executor identity, status, attempt/usage counters, workflow coverage, aggregate artifact status, and whether result-body persistence is enabled. It does not read or return a result body. `view: "children"` applies to workflow controllers and lists at most 100 bounded child-attempt records, including status, invocation, safe references, usage, and sanitized error markers; on an Agent or manual task it says that children are not applicable.

After a reload, a settled workflow can be resolved from its persisted aggregate metadata and internal manifest. For `wf_*` summary/children lookups, a live canonical workflow binding wins, followed by a validated persisted `workflowAggregate.taskBinding`, then an unbound aggregate manifest; `metadata.workflowId` is consulted only for old tasks with no aggregate. The manifest reader validates the fixed session-owned location and the Todo binding, but this is not a public artifact API: there is no standalone `ArtifactRead` tool, and `result`/body views are not implemented. Privacy-off summary and children views never disclose result, prompt, log, or child error text; they use the fixed `Output persistence disabled.` marker where an error must be represented. Workflow children are bounded attempts, not verification evidence or proof of task completion.
